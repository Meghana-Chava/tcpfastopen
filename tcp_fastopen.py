...
def run_performance_tests(net):
    h1, h2 = net.get('h1', 'h2')
    client = h1.popen("time sudo wget -r {0}/http/{1}".format(h2.IP(), args.host_server + '.html'), shell=True, stdout=PIPE, stderr=PIPE)
    #client = h1.popen("time sudo wget http://{0}".format(args.host_server), shell=True, stdout=PIPE, stderr=PIPE)
